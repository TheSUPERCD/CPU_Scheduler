...
std::string showstate(pstate_t state){
    switch(state){
...
bool p_comp_burst(const process_t *p1, const process_t *p2){
    return (p1->burst_duration < p2->burst_duration);
}

bool p_comp_priority(const process_t *p1, const process_t *p2){
    return (p1->priority < p2->priority);
}

ProcessList::ProcessList(std::ifstream& datafile){
...
void ProcessList::display(process_type_t scheduler_type, int num_procs_to_display=-1){
...
void ProcessList::FIFO_scheduler(){
    bool isBusy = false;
    int current_time = 0;
...
void ProcessList::SJF_scheduler(){
    bool isBusy = false;
    int current_time = 0;
    int completed_procs = 0;
    int next_proc_idx = 0;
...
void ProcessList::PRIORITY_scheduler(){
    int current_time = 0;
    int completed_procs = 0;
    int next_proc_idx = 0;
    std::vector<process_t *> proc_queue;
